    ...
    ">
                  ...
                  <div class="diago-alarm-cover1">
                    ...
                    <div class="diago-border2 ml-2">
                      ...
                      <v-date-picker v-model="date2" class="date-picker ml-10" :event-color="(date) => (date[9] % 2 ? 'red' : 'yellow')
                        " readonly no-title color="#828c8f" width="267" height="200"></v-date-picker>
                    </div>
                  </div>
                  <div class="diago-btn-cover mt-1 mb-3 mr-4 ml-3">
                    <v-btn color="#de8788" class="diago-btn" max-height="24px" max-width="95px" rounded
                      :to="'?cam=' + focusCAMID + '&main=' + menuData.main + '&secondary=' + menuData.secondary + '&tab:tab-1'">
                      <span class="diago-btn-font" @click="VideoActive('tab-2')">查看更多</span>
                    </v-btn>
                  </div>
                </div>
                <!-- ROI物件設定視窗 -->
                ...
                <div class="diago-tootip-img" :class="[
                  ...
                  <div class="pa-3 pe-1 diago-tootip-img-e">
                    <img id="streamingVIS" src="/images/Group 2453.png" width="100%" />
                  ...
                  <div class="pa-3 ps-1 diago-tootip-img-e">
                    <img id="streamingIR" src="/images/Group 2454.png" width="100%" />
                    <canvas id="diago-tootip-canvas2" class="pixi-cramre pa-3"></canvas>
                    <PixijsDisplayData :size="'diago-tootip-canvas2'" :canvasId="'diago-tootip-canvas2'" v-if="diagoTootipCanvas"
                                    :roidata="roiDataToKonve" :camId="focusCAMID === null ? $route.query.cam : focusCAMID" />
                  </div>
                  ...
                  <v-card class="camera-bg col-windows-ul align-items-c mt-3" style="">
                    ...
                    <ul id="sortable" :class="sortable">
                      <li v-for="(item01, index01) in cam" :key="index01"
                        ...
                        :id="`diagoHover`" @click="(e) => { startGetROI(item01.cam_id, e), focusCamName = item01.name }">
...
import MultiScreenstand from '../components/MultiScreen/MultiScreen-stand-bak.vue'
...
export default {
    ...
    focusCAMID(data) {
      if (data !== null) {
        this.wsTOway.send(JSON.stringify({
          ...
          content: {
            "cam_id": data,
            "status": 0 // "off" = 關閉 super resolution ; "on" = 開啟 super resolution
          }
        }))
      }
    },
  ...
  mounted() {
    // setInterval(()=>{
    //   console.log(document.getElementById('diago-tootip-canvas'));
    // },1000)
    ...
    // setTimeout(() => {
    //   this.autoOpenWindow('cam_00_00008')
    // },3000)
  },
  updated() {
    // 判斷視窗該在哪個方位
  ...
  methods: {
    ...
    connectTOWayWS() {
      ...
      this.wsTOway = new WebSocket('ws://localhost:8703')
      this.wsTOway.onopen = (e) => {
    ...
    // alarmStatus(e){
    //   // var result = this.alarmCam.includes(e)
    //   return true
    // },
    // 初始化KonvaForwebWorker
    // initKonva[指定canvas id,camera id]
    initKonva(canvasCustom, camID) {
      var htmlCanvas = document.getElementById(canvasCustom);
      // htmlCanvas.width = 600;
      // htmlCanvas.height = 450;

      var hasOffscreenSupport = !!htmlCanvas.transferControlToOffscreen;
      if (hasOffscreenSupport) {
        var offscreen = htmlCanvas.transferControlToOffscreen();
        // console.log(offscreen)
        var webTempSocket = new Worker('./worker/konvaWorker.js');
        // pass canvas into webworker, so we can do all rendering inside it
        webTempSocket.postMessage({ canvas: offscreen }, [offscreen]);
        webTempSocket.addEventListener('message', (e) => {
          // console.log('回傳', e.data)
          var data = e.data
          if (data.type === 'move') {
            ...
            this.ws.send(JSON.stringify(data.response))
          }
        })
        // "proxy" all DOM events from canvas into Konva engine

        this.weblist.push({ name: camID, ws: webTempSocket })
      } else {
        htmlCanvas
          .getContext('2d')
          .fillText(
    ...
    openSocket() {
      this.ws = new WebSocket(this.$config.webSocket)
      // this.ws = new WebSocket('ws://localhost:8701')
      this.ws.onopen = () => {
        console.log('連線成功')
        this.webStatus = '連線成功'
        if (this.reMeida) {
          console.log('重新載入影像')
          this.webStatus = '重新載入影像'
      ...
      this.ws.onclose = () => {
        ...
        this.webStatus = '斷線'
        if (this.wsblock) {
          this.restartDisplay1 = setTimeout(() => {
            console.log('重新嘗試連線')
            this.webStatus = '重新嘗試連線'
            this.openSocket()
            this.reMeida = true
    ...
    submitForm() {
      ...
      const status = this.checkbox1
      const threshold = this.threshold
      const thisSpots = this.tpmedata.spot
    ...
    VideoActive(page) {
      // this.tab = page
      this.$router.push({ query: { cam: this.focusCAMID, main: this.$route.query.main, secondary: this.$route.query.secondary, tab: 'tab-2' } }, () => {
    ...
    getCameraList() {
      axios({
        ...
        .then((params) => {
          ...
          if (this.tab === 'tab-1') {
            this.$router.push({ query: { cam: this.CamList[0].cam_id, main: this.menuData.main, secondary: this.menuData.secondary, tab: 'tab-1' } })
          }
    ...
    showDisplay(output) {
      ...
      this.ws.onmessage = (event) => {
        ...
        const canvas13 = document.getElementById(`streamingIR`)
        ...
        // this.wsdata = data[0].img
        // var start1 = (this.CamPage - 1) * displayTotle
        // var end1 = start1 + displayTotle
  ...
  beforeDestroy() {
    ...
    // console.log('umount')
    // if (this.alarmlistIntID !== null) {
    //   clearInterval(this.alarmlistIntID)
    //   this.alarmlistIntID = null
    //   console.log('Destroy:alarmlistIntID')
    // }
    if (this.getRoiInterval !== null) {
      clearInterval(this.getRoiInterval)
      this.getRoiInterval = null
