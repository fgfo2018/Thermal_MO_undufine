...
self.onmessage = function (evt) {
    ...
    if (evt.data.roidata) {
        ...
        // blob
        // runRoiblol(evt, camid, width1, height1, 0, 0) // [data, camid, width, height, 行, 列]
        // spot
        runRoispot(roi, camid, width1, height1, 0, 0)
        // line
        runRoiline(roi, camid, width1, height1, 0, 0)
        // line
        runRoiscope(roi, camid, width1, height1, 0, 0)
        //! 測試第二個
        // blob
        var index1 = 1
        for (var i = 0; i < 3; i++) {
            for (var z = 0; z < 4; z++) {
                runRoiblol(evt, 'cam_00_000' + ("0" + index1).slice(-2), width1, height1, i, z)
                index1++
            }
        }

        // spot
        // runRoispot(roi, camid, width1, height1, 1, 2)
        // line
        // runRoiline(roi, camid, width1, height1, 1, 2)
        // line
        // runRoiscope(roi, camid, width1, height1, 1, 2)
    }
    if (evt.data.canvas) {
...
var tmpblobkey = {}
var tmpblobdata = {}
async function runRoiblol(event, camid, width1, height1, row, list) {
    const imgBlob = await fetch('data:image/png;base64,' + event.data.slashY).then(
        (r) => r.blob()
    );
    if (tmpblobkey[camid] === undefined) {
        tmpblobkey[camid] = []
    }
    if (tmpblobdata[camid] === undefined) {
        tmpblobdata[camid] = []
    }
    // use "createImageBitmap" instead of "new window.Image()"
    const img = await createImageBitmap(imgBlob);
    ...
    blob.forEach((index) => {
        ...
        index.points.forEach((e1, arrayindex) => {
            ...
            var sch = tmpblobdata[camid].find((item) => JSON.stringify(item.points) === JSON.stringify(pr))
            ...
            ar.push({
                ...
                name: `blobFind${index.blob_number}_${associated}_${row}_${list}`,
                fill: fill1,
    ...
    // 編號
    var array = []
    tmpblobkey[camid].forEach((index) => {
    ...
    tempArr.forEach((index) => {
        ...
        if (findofblob2.length > 0) {
            ...
            var simpleText = new Konva.Text({
                ...
                offsetX: 10 + ~width1 * list,
                offsetY: 6 + ~height1 * row,
                align: 'center'
            });
            layer.add(simpleText);
            simpleText = null
...
async function runBunnies() {
    ...
    function update() {
        ...
        // if (fps % 33 === 0) {
            ...
            // end
            var path = []
            Object.keys(tmpblobkey).forEach((key) => {
                tmpblobkey[key].forEach((index) => {
                    var path1 = stage.find(index)
                    path1.forEach((index2) => {
                        path.push(index2)
                    })
